...
// Request pattern: /p/a25fcf8d/proxy/{encodeURIComponent(targetUrl)}
export async function onRequest(context) {
  const { request } = context;
  const url = new URL(request.url);
  const proxyPath = url.pathname.replace('/p/a25fcf8d/proxy/', '');
    ...
    if (ct.includes('mpegurl') || ct.includes('x-mpegurl') || /\.m3u8?$/i.test(target)) {
      ...
      const rewritten = lines.map(line => {
        ...
        try {
          ...
          return '/p/a25fcf8d/proxy/' + encodeURIComponent(abs);
        } catch { return line; }
      });
